...
def MultisourceOrthomosaicFactory(
    ...
    for source_name in data_source_config_parameters[train_validation_test]:
        if data_source_config_parameters[train_validation_test][source_name]:
            ...
            if "BDA" in model_hyperparameters["task"]:
            ...
            initialized_orthos = OrthomosaicFactory(
                ...
                statistics_file_path=(
                    dataset_paths_dict["statistics"]
                    if "statistics" in dataset_paths_dict
                    else None
                ),
                backend=backend,
                limit=limit,
...
def OrthomosaicFactory(
    ...
    for target_file in target_files:  # pylint: disable=too-many-nested-blocks
        ...
        try:
            if (
                limit is None
                or (isinstance(limit, int) and len(oms) < limit)
                or (isinstance(limit, str) and limit in geotif_name)
            ):
                ...
                # Load the affine transformer, GSD, and Timestamp if possible
                if not orthomosaic_stats is None:
                    stats_gsd = float(
                        orthomosaic_stats.loc[geotif_name]["GSD (m/px)"]
                    )  # divide by 100 to convert to meters per pixel.
                    gsd_x = stats_gsd * scale_factor
                    gsd_y = stats_gsd * scale_factor

                    # Extract Timestamp for the orthos...
                    ortho_timestamp = str(
                    ...
                    year = int(year)
                    month = int(month)
                    hour = 14  # TODO: WE ASSUME 2PM LOCAL TIME FOR ALL ORTHOS, THIS NEED TO RELFECT THE ACTUAL TIME

                    # SATMAE applies an offset to the timestamps, so the timestamp should be offset similiarly
                    if timestamp_offset_strategy:
                        # NOTE: this is indexed on the minimum year from the satmae paper
                        # TODO: - consider what should be done here (We may consider a different offset?)
