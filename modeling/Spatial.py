...
def BuildingFactory(bda_annotations, label_source=None):
    ...
    for building in bda_annotations:
        ...
        if "payload_version" in building.keys():
            if building["payload_version"] == "1.1":
                buildings.append(Building(identifier=building["id"],
                                          ...
                                          adjustment_subfield=None,
                                          label_source=label_source))
            if building["payload_version"] == "2.0":
                ...
                raise ValueError("Unknown Payload Version: " + str(building["payload_version"]))
        else:
            buildings.append(Building(identifier=building["id"],
                                      label=building["label"],
                                      geometry_source=geometry_source,
