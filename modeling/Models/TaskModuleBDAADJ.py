...
from modeling.Models.ModelDatum import Y_HAT_SEGMENTATION_UNMASKED, DISPLACEMENT_FIELD, DO_SOFTMAX, CHANNEL_INPUT
...
class TaskModuleBDAADJ(TaskModuleBDA):
    ...
    def format_batched_data_to_model_input(self, batch):
        ...
        if self._include_gsd:
            t, gsd = super().format_batched_data_to_model_input(batch)
            return t, buildings, gsd
        t = super().format_batched_data_to_model_input(batch)
        return t, buildings
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_size, _, width, height = x[CHANNEL_INPUT][0].shape
        ...
        model_output = self.model.forward(x)
